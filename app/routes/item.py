...
async def get_items(
    filters: ItemFiltersSchema = Depends(), service: ItemService = Depends()
):
...
async def update_item(
    item_id: UUID, schema: ItemUpdateSchema, service: ItemService = Depends()
):
    return await service.update(item_id, schema)


@router.delete(
    "/{item_id}", status_code=204, dependencies=[ItemAccessService.validate_delete()]
)
